...
@foreach ($students as $studentId => $records)
   @php
        $first = $records->first(); // A sample record for shared data like name, roll number
   @endphp
    ...
    <table class="full-width">
        <tr>
            ...
            <td class="center-text">
                ...
                <h4 class="admission-ticket">REAPPEAR ADMISSION TICKET</h4>
            </td>
     ...
     <div class="heading">
        <table class="full-width line-height-small">
            <tr><td><strong>CENTRE:</strong></td><td class="uppercase">{{ $first->institute->name ?? '-' }}</td></tr>
            <tr><td><strong>COURSE:</strong></td><td class="uppercase">{{ $first->program->name ?? '-' }}</td></tr>
            <tr><td><strong>ROLL NO:</strong></td><td>{{ $first->roll_number }}</td></tr>
            <tr><td><strong>STUDENT NAME:</strong></td><td class="uppercase">{{ $first->student_name }}</td></tr>
            <tr><td><strong>EXAM TYPE:</strong></td><td>Reappear</td></tr>
            <tr><td><strong>ACADEMIC YEAR:</strong></td><td>{{ $first->academic_year ?? '2024-25' }}</td></tr>
            <tr><td><strong>SEMESTER:</strong></td><td>{{ $first->semester ?? '-' }}</td></tr>
             <tr>
                <td><strong>REAPPEAR SUBJECTS:</strong></td>
                <td>
                    {{ $records->pluck('course_code')->join(', ') }}
                </td>
            </tr>
